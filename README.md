...
| **② 模型「偷懒占位符」** | 输出 `...` / `[content here]` / `<!-- 其余略 -->` | Generator Prompt 用 v0 同款**强约束**（零占位符、必须完整、`<!--EOF-->` 收尾） |
...
### 主路线（推荐，3 节点）— 适用于绝大多数单页

```
┌─────────┐     ┌──────────────┐     ┌────────────────┐     ┌──────────┐
│  Start  │────▶│  LLM-1       │────▶│  LLM-2         │────▶│  Output  │
│ 一句话   │     │  Planner     │     │  Generator     │     │  html    │
│ 需求     │     │  (出大纲JSON)│     │  (出完整HTML)  │     │          │
└─────────┘     └──────────────┘     └────────────────┘     └──────────┘
                 结构化输出            引用 {{planner_output}}
                 temp=0.3             temp=0.6~0.7, max_tokens拉满
```

- **LLM-1 Planner**：只规划页面结构（哪些 section、每个 section 的真实文案要点、视觉意图、配色），输出 JSON。**不出代码**。
- **LLM-2 Generator**：把 Planner 的大纲作为强约束，**单次**生成完整 HTML。
...
├── prompts/
│   ├── system-prompt.md            ← 全局 System Prompt（Planner 与 Generator 共用）
│   ├── planner-prompt.md           ← LLM-1 Planner 的 User Prompt
│   ├── generator-prompt.md         ← LLM-2 Generator 的 User Prompt（核心）
...
|---|---|---|---|---|---|
| LLM-1 Planner | GLM-4.6 / 4.5 | **0.3** | 默认即可 | **开**（粘 generator-schema.json） | 只要结构稳，温度低 |
| LLM-2 Generator | GLM-4.6（首选）| **0.6~0.7** | **尽量拉满**（≥16000，按 UI 上限） | **关**（输出纯 HTML） | 要创意但不发散 |
...
3. 加 **LLM-1 Planner**：粘 system-prompt + planner-prompt，开结构化输出粘 schema，temp=0.3。
4. 加 **LLM-2 Generator**：粘 system-prompt + generator-prompt，把 Planner 输出变量拖进 `{{planner_output}}`，temp=0.7，max_tokens 拉满，关结构化输出。
5. 加 **Output 节点**：选 Generator 的输出，命名为 `html`。
6. 右上角「运行」用一句话测试，如：*「帮我做一个 AI 写作助手的落地页」*。
7. 发布 → 访问 API（见 api-and-preview.md）。
...
- **Generator 偶发截断**（极少见，因 GLM 窗口大）：
  - 方案 A：在调用端检测末尾是否有 `<!--EOF-->`，没有则触发续写循环（把已生成内容作为 assistant 消息回传 + "continue exactly where you left off"）。
