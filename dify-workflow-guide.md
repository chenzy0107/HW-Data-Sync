...
- 预期：Output 节点输出一整段以 `<!DOCTYPE html>` 开头、`<!--EOF-->` 结尾的完整 HTML。
- 把它存成 `.html` 文件用浏览器打开，应是一个设计完整、内容丰富的落地页。
...
2. 加 **「迭代 (Iteration)」节点**：
   ...
   - **循环内部**：放一个 **LLM 节点**，Prompt 大意：
     ...
     > 要求：完整、真实文案、无占位符、用 Tailwind class。
...
3. ⚠️ **加 Template 节点（关键，不可省）**：
   ...
   - 加 **「模板 (Template / Jinja2)」节点**，内容：
     ...
     <!--EOF-->
     ```
     （把 Iteration 的片段数组用 `{% for %}` 或 `| join` 拼进骨架。`title` / `primary` 等可从 Planner 的 theme 取。）
...
| Generator 输出被截断（没有 `<!--EOF-->`） | `max_tokens` 太小。拉到上限；或改走兜底 Iteration 路线。 |
