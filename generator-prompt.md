...
1. **必须是单个完整 HTML 文件**，以 `<!DOCTYPE html>` 开头。结构：
   ```html
   <!DOCTYPE html>
   <html lang="zh-CN">
   <head>
     ...
     <style>:root{ --c-primary:...; --c-secondary:...; --c-accent:...; } /* 全局样式、动画、滚动渐入等 */</style>
   </head>
   <body>
     <!-- 每个区块用 <section> 包裹，并给注释标记，如 <!-- SECTION: hero --> -->
     ...
     <footer>...</footer>
     <script>/* 内联交互脚本：移动端菜单、滚动渐入、计数动画等 */</script>
   </body>
   </html>
   <!--EOF-->
   ```
...
3. **内容必须真实且饱满**：
   ...
   - **绝对禁止**出现"示例文字""待补充""Lorem Ipsum""……""[content here]""<!-- 其余略 -->"等任何占位符或省略。
...
- ❌ 不要在 HTML 中间截断——必须写完并以 `<!--EOF-->` 结尾。

现在，直接输出这个完整的 HTML 文件，从 `<!DOCTYPE html>` 开始，到 `<!--EOF-->` 结束。把大纲里的每一个区块都写完整、写丰富。
