...
| **⑤ 一拉到底/只能看** | 页面是线性宣传册，缺乏可操作交互 | **工具型优先**：Planner 必选 3+ 个强交互区块（勾选器 checkbox_selector、智能推荐 recommender、对比表 comparison_table、分步向导/配置器/计算器），强交互放首屏焦点位；Generator 用内联原生 JS 实现勾选联动→智能推荐闭环；schema 的 `interaction`+`recommend_rules`/`score_axes` 等字段支撑 |
