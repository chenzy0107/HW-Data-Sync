...
> 作用：Generator 现在输出「核心功能说明 + ```html 代码块」两部分（自然衔接，**无分隔标记**）。本 Code 节点把它拆成 **`summary`**（说明文字）和 **`html`**（纯 HTML）两个独立字符串变量，供下游 Output 节点分别引用。
>
...
function main({ raw }) {
  ...
  if (block) {
    ...
    // html = 代码块内部内容，剥围栏，截 <!DOCTYPE html> 到 </html>
    let h = block[1].trim();
    ...
    // 兜底：没有 ```html 代码块，整体当 html 处理（极少见）
    html = text.trim();
