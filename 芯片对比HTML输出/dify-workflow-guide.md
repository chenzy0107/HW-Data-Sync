...
## 主路线（6 节点 · 含私有 RAG 知识库 + 设计总结）

> 拓扑：Start → LLM-0 Query Rewriter → 知识检索 → LLM-1 Planner → LLM-2 Generator(总结+HTML) → Code 拆分 → Output(summary + html)
>
> 节点编号对照：下面「节点 1~7」分别对应 Start、LLM-0、知识检索、LLM-1、LLM-2、Code 拆分、Output。
...
- 注意：该 Prompt 让 Generator **先输出 ≤500 字设计总结，再输出 HTML**，两段用 `===SUMMARY===` / `===HTML===` 标记隔开。这是预期的，下一步用 Code 节点拆开。
- 变量绑定：
...
**⑤ 结构化输出**
- **关闭**（Generator 要输出"总结+HTML"文本流，不是 JSON）。

**⑥ 参数**
- `temperature`：**0.6 ~ 0.7**。
- `max_tokens`：**拉到 UI 允许的上限**（GLM-4.6 可设很大；务必远大于默认 4K，否则单页 HTML 会截断）。

### 节点 6：Code 节点（拆分 summary 与 html）

在 Generator 后面加一个 **「代码执行 (Code)」节点**，命名 `Split`。它把 Generator 的混合输出拆成 `summary` 和 `html` 两个独立变量。

**① 语言**：JavaScript（或 Python，脚本见 `prompts/code-split-node.md`）。

**② 输入变量**：
- 变量名：`raw` → 绑定 **Generator 节点的输出**（Generator 的 text / output）。

**③ 代码**：直接粘贴 `prompts/code-split-node.md` 里的 JS 版（或 Python 版）代码。

**④ 输出变量**（在节点「输出」里定义）：
- `summary`：String
- `html`：String

> 作用：Dify 的 LLM 节点只产出一个文本流，Output 节点没法自动区分哪段是总结、哪段是 HTML。这个 Code 节点按 `===HTML===` / `===SUMMARY===` 标记拆开，并顺手剥掉 HTML 的 markdown 围栏，是最稳的方式（不依赖 JSON Schema，避免长 HTML 字符串解析失败）。
>
> 验证：点节点「运行」，`summary` 应是纯中文说明、`html` 应以 `<!DOCTYPE html>` 开头。

### 节点 7：Output（输出 / End）

在 Split 后面加 **「输出 (Output / End)」节点**。现在输出**两个**变量：

- 添加输出变量 1：
  - 变量名：`summary`
  - 值：选 **Split 节点的 `summary`**。
- 添加输出变量 2：
  - 变量名：`html`
  - 值：选 **Split 节点的 `html`**。
- API 最终返回 `data.outputs.summary`（设计说明）和 `data.outputs.html`（完整 HTML）。
...
- 预期：Output 节点输出**两个**变量——`summary`（≤500 字中文设计说明）、`html`（以 `<!DOCTYPE html>` 开头、`</html>` 结尾的完整 HTML，无注释）。
...
>
> ⚠️ **兜底路线也必须输出 `summary` + `html` 两个变量**（与主路线一致），否则前端取不到 summary。下面用「主路线的 Generator 同时负责 summary」的方式补齐。
...
Start → Query Rewriter → 知识检索 → Planner(输出 sections 数组)
      ...
      → Template 节点(拼成完整 HTML 骨架)
      → 主路线的 Generator(出 summary + html)  ★复用,负责总结与最终格式
      → Code 拆分 → Output(summary + html)
```

> 说明：兜底路线的分块由 Iteration 完成（避免截断），但最终仍走一遍主路线的 Generator——它接收 Iteration 拼好的完整 HTML，**生成 ≤500 字设计说明并按 `===SUMMARY===`/`===HTML===` 格式重新输出**，再由 Code 节点拆分。这样保证两条路线的 Output 完全一致。
...
3. ⚠️ **加 Template 节点（关键，不可省）**：
   ...
   - 加 **「模板 (Template / Jinja2)」节点**，内容如下（不含任何注释；这里用 4 个反引号 ```` 包裹示意，实际粘贴时按 Jinja2 模板语法填，外层不要套 markdown 围栏）：
   ...
   - 把 Iteration 的片段数组用 `{% for %}` 或 `| join` 拼进骨架。`title` / `primary` / `footer_copy` 等可从 Planner 的 theme 取。各 section 片段本身也不得含注释。**注意：Template 节点输出的就是纯净 HTML 字符串，不要再外包 markdown 围栏**（围栏由下一步 Generator 加）。
   - 或用 **Code 节点**（JS）：`return { html: "<!DOCTYPE html>..." + inputs.sections.join("\n") + "..." }`。注意 Code 节点字符串上限 40 万字符。
4. **复用主路线的 Generator 节点**（出 summary + html）：
   - 把上一步 Template 输出的完整 HTML，作为变量（如 `{{assembled_html}}`）注入主路线 Generator 的 USER Prompt（替换原来的 `{{planner_output}}`，或新增一段"已生成的完整 HTML 如下，请基于它写设计说明"）。
   - Generator 据此生成 ≤500 字设计说明，并按 `===SUMMARY===`/`===HTML===` 格式重新输出（HTML 部分可直接采用 `{{assembled_html}}`，外包 ```html 围栏）。
   - 这样保证两条路线的输出结构一致。
5. **Code 拆分 + Output**：与主路线完全相同。
...
- **兜底路线必须也输出 summary**：不要跳过步骤 4（复用 Generator），否则 Output 缺 summary、前端显示空。
...
| 配色每次都一样/像固定模板 | Planner 没随机派生。检查 Planner 输出的 `theme.base_hue` 是否每次不同、`color_strategy` 是否变化；强化 planner-prompt 的「配色多变」要求。 |
| **★页面一拉到底/没有强交互** | Planner 规划的还是展示型区块（hero/features/pricing），或 Generator 没实现强交互。① 检查 Planner 输出的 sections 是否有 **3+ 个强交互类型**（checkbox_selector/recommender/comparison_table/stepper_wizard/configurator/calculator）；② 若没有，强化 planner-prompt 的「工具型优先」要求（必要时直接在 user_requirement 里点明要哪种交互，如"做一个能勾选需求并推荐方案的页面"）；③ 确认 recommender 紧邻 checkbox_selector 之后。 |
| **★推荐/勾选联动不生效** | Generator 没实现联动逻辑。① 检查 Planner 给 checkbox_selector 的 items 是否带 `impact`、recommender 是否带 `recommend_rules`；② Generator 的推荐逻辑应是纯函数 `recommend(selected)`，强化 generator-prompt 的实现要求。 |
...
| **★暗色切换不生效** | 模型没用 CSS 变量+`data-theme` 方案，而是写死颜色。强化要求用 `:root[data-theme="dark"]` 覆盖 CSS 变量。 |
...
| **★Output 里没有 summary，只有 html**（或反之） | Generator 没按 `===SUMMARY===`/`===HTML===` 格式输出，或 Code 拆分节点没配好。① 检查 Generator 原始输出是否含两个标记行；② 检查 Code 节点输入 `raw` 是否绑了 Generator 输出、输出变量是否定义了 `summary` 和 `html`；③ Output 节点是否两个变量都加了。 |
| **★summary 和 html 混在一起 / 拆不开** | Code 节点没生效或代码没粘对。重新粘贴 `prompts/code-split-node.md` 的脚本，确认语言（JS/Python）与入参名一致。 |
| summary 超过 500 字 | Generator Prompt 的字数约束没生效，强化提示或检查温度（总结部分温度宜低）。 |
