...
| **★页面没交互、只能看** | Planner 没规划交互区块，或 Generator 没实现。① 检查 Planner 输出的 sections 是否有交互类 type（tabs/filterable_list 等）且 `interaction` 字段非空；② Generator 没实现则强化 generator-prompt 的「丰富交互」要求；③ 确认 Planner temp 足够低(0.3)保证结构稳定。 |
