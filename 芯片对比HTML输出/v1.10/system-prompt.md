...
【任务】作为全局 SYSTEM 规范，定义「产出能在浏览器直接、完整、正确渲染、且达标的单页交互 HTML」所需的全部硬约束（最高目标/角色/成功标准/设计自由/数据策略/运行安全/技术骨架），供 Planner 与 Generator 节点引用并逐条兑现。
...
- 避免引用外部图片/字体 CDN：外链图片/字体在沙箱或弱网下可能加载失败，导致版面残缺、空白图裂；视觉表达优先用 **CSS 渐变、内联 SVG、emoji、系统字体**，确需图标再用允许的图标库 CDN。
...
6. 自定义样式写进 `<style>`（变量、keyframes、玻璃态等），不要用 `<link rel="stylesheet">` 引外部 .css（外部图标库除外）。⚠️ 本方案用 **Tailwind Play CDN（浏览器内 JIT 编译）**，`<style>` 中**禁用 `@apply`**（Play CDN 不处理 `@apply`，写了也不生效，样式会静默丢失）；需要复用样式直接写原生 CSS 或 Tailwind 工具类/任意值。
7. 禁用调试代码：不写 `console.*`/`debugger`，不在页面渲染调试文字（test/TODO/调试中），不把思考/规划过程作为可见内容输出。
8. 代码不放注释（HTML/CSS/JS 均不放），思考过程不进页面。（上方骨架模板中的 `<!-- ①… -->` 编号注释仅为顺序示意，最终生成的 HTML/CSS/JS 中不得保留任何注释。）
