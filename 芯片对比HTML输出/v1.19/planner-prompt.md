...
- 风格（G）：据主题与对比实体自主判断 `theme.vibe`，每次显著不同、不挑安全套路、不总用蓝紫（见 SYSTEM-G）。
...
- 区块填写：每区块写清 `layout`（布局位置/角色）、`purpose`（作用/类型）、`heading`、`content_packing`（如何填满版面）、`copy`（文案要点，事实取自 Context，缺的标「待定」）、`interaction`（交互行为 + 与谁联动）。配色给具体色值、与 vibe 匹配（见 SYSTEM-G）。

【输出】严格按 JSON Schema 输出（见结构化输出配置），不输出 HTML/解释/前后缀文字。

【字段说明（与 Schema 一致）】
- `theme.brand_name`：优先取自 Context；无明确品牌名时用中性页面标题/工具名，不编造公司全称。
- `theme.vibe`：设计风格/气质，据主题与对比实体自主判断，每次显著不同、不挑安全套路（对应硬约束 G）。
